...
export default function DeliveryPage(){
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-white flex items-center gap-2"><Truck className="h-5 w-5" /> Delivery & Logistics — OTP + POD <Badge>Paid</Badge></h1>
        ...
        {DELIVERIES.map(d=>(
          <Card key={d.id}>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="rounded-xl border p-2 bg-zinc-800/60 text-xs space-y-1">
      ...
      <Card className="p-4">
        ...
        <div className="mt-2 grid grid-cols-6 gap-2 text-xs text-center">
          {["500×40", "200×25", "100×38", "50×12", "20×10", "10×15"].map(d=>(
            <div key={d} className="rounded-xl border p-2 bg-zinc-900"><div className="font-bold">{d}</div><div className="text-zinc-400">notes</div></div>
          ))}
        </div>
        <div className="text-xs text-zinc-400 mt-2">Collection ₹42,800 vs deposit ₹42,800 — <span className="text-orange-500 font-semibold">Matched (no mismatch)</span></div>
      </Card>
    </div>
  )
}
