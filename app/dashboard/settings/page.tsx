...
export default function SettingsPage(){
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-white">Settings & Billing — Paid</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-3 text-sm">
            <div className="rounded-xl border p-4 bg-gradient-to-br from-zinc-900 to-zinc-800 text-white">
              <div className="flex items-center justify-between"><span className="font-bold">{plan.name} — Paid</span><Badge className="bg-orange-500 text-white">Active • Trial 14d</Badge></div>
            ...
            <div className="text-xs text-zinc-400">Paid includes: managed DB, S3, backups, WAF, SSL, GST updates.7347.missing</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            <label className="block"><span className="text-xs font-medium">Business Name</span><input defaultValue="SKS Distributors Pvt Ltd" className="mt-1 w-full rounded-xl border border-zinc-700 px-3 py-2 bg-zinc-900 text-white" /></label>
            <label className="block"><span className="text-xs font-medium">GSTIN • GSP</span><input defaultValue="33ABCDE1234F1Z5 — ClearTax" className="mt-1 w-full rounded-xl border border-zinc-700 px-3 py-2 bg-zinc-900 text-white" /></label>
            <label className="block"><span className="text-xs font-medium">Branches</span><input defaultValue="Salem HQ, Salem-B, Erode, Namakkal, Hosur" className="mt-1 w-full rounded-xl border border-zinc-700 px-3 py-2 bg-zinc-900 text-white" /></label>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <Card><CardHeader className="pb-2"><div className="font-semibold text-sm flex items-center gap-2"><Bell className="h-4 w-4" /> Notifications</div></CardHeader><CardContent className="text-xs space-y-1"><label className="flex justify-between border rounded-xl px-3 py-2"><span>WhatsApp (Gupshup)</span><span className="text-orange-500">● ON</span></label><label className="flex justify-between border rounded-xl px-3 py-2"><span>SMS (MSG91)</span><span className="text-orange-500">● ON</span></label><label className="flex justify-between border rounded-xl px-3 py-2"><span>Email (Resend/SES)</span><span className="text-orange-500">● ON</span></label></CardContent></Card>
        <Card><CardHeader className="pb-2"><div className="font-semibold text-sm flex items-center gap-2"><Globe className="h-4 w-4" /> Platform Mode</div></CardHeader><CardContent className="text-xs space-y-2"><div className="rounded-xl border p-3 bg-zinc-800/60"><div>PLATFORM_MODE=<b>internal</b></div><div>MULTI_TENANT=false • SUBSCRIPTIONS_ENABLED=true (paid)</div><div>PUBLIC_SIGNUP=customer+retailer • STAFF_SIGNUP=false</div></div><div className="flex items-center gap-1 text-orange-500"><Shield className="h-3.5 w-3.5" /> Paid hardening: WAF, VPC, TLS 1.3</div></CardContent></Card>
      </div>
    </div>
  )
}
