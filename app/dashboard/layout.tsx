  ...
  return (
    <div className="min-h-screen bg-black flex">
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-[280px] bg-zinc-900 text-zinc-100 flex flex-col border-r border-zinc-800 transition ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        ...
        <div className="p-3">
          <div className="rounded-xl bg-gradient-to-br from-orange-500 to-amber-500 p-3 text-white">
            ...
            <div className="mt-2 h-1.5 rounded-full bg-white/20 overflow-hidden"><div className="h-full bg-white" style={{ width: "68%" }} /></div>
        ...
        <nav className="flex-1 overflow-y-auto p-2 space-y-1">
          {NAV.map(n => {
            ...
            return (
              <Link key={n.href} href={n.href} onClick={()=>setOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm ${active ? "bg-orange-500 text-white font-semibold" : "text-zinc-300 hover:bg-zinc-800 hover:text-white"}`}>
        ...
        <div className="border-t border-zinc-800 p-3">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-full bg-orange-500 text-white grid place-items-center font-bold text-xs">{user?.avatar || "GR"}</div>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-zinc-800 bg-black/90 backdrop-blur px-4">
          <button onClick={()=>setOpen(true)} className="lg:hidden p-2 rounded-lg border border-zinc-700 text-white"><Menu className="h-4 w-4" /></button>
          <div className="hidden md:flex items-center gap-2 text-xs">
            <Badge variant="secondary">PAID</Badge>
            <span className="text-zinc-400">Salem HQ • Branch: Salem-A • Today: {new Date().toLocaleDateString("en-IN")}</span>
            <span className="hidden lg:inline-flex items-center gap-1 ml-2"><span className="h-2 w-2 rounded-full bg-orange-500 animate-pulse" /> Live — Redis • Postgres • Meilisearch</span>
          </div>
