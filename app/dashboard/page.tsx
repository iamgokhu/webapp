    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-400">Paid — Enterprise • Single owner controls all 35 modules, 20 agents, 5 branches. Everything auditable.</p>
        </div>
          ...
          <Card key={c.k} className="relative overflow-hidden">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="text-[11px] font-semibold tracking-widest text-zinc-400">{c.k}</div>
                <c.icon className={`h-4 w-4 ${c.ok ? "text-orange-500" : "text-amber-500"}`} />
              </div>
              <div className="mt-1 text-xl font-bold">{c.v}</div>
              <div className={`text-xs ${c.ok ? "text-orange-500" : "text-amber-600"}`}>{c.sub}</div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2"><div className="font-semibold text-sm">Orders by Channel — Today</div><div className="text-xs text-zinc-400">483 orders • Central validation: Order Control Engine</div></CardHeader>
          <CardContent className="h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            <div className="flex items-center justify-between rounded-xl border px-3 py-2"><span>Billing globally</span><span className="font-bold text-orange-500">● ON</span></div>
            <div className="flex items-center justify-between rounded-xl border px-3 py-2"><span>Orders — All channels</span><span className="font-bold text-orange-500">● ON</span></div>
            ...
            <div className="text-xs text-zinc-400">Toggles apply instantly via Order Control Engine (hierarchical blocking: Global → Category → SKU → Channel).</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            {[
              ...
              <div key={a.t} className="flex gap-3 rounded-xl border px-3 py-2 bg-zinc-900">
                <AlertTriangle className={`h-4 w-4 mt-0.5 ${a.sev==="high"?"text-red-500":"text-amber-500"}`} />
                <div className="flex-1"><div className="font-medium leading-none text-xs">{a.t}</div><div className="text-xs text-zinc-400">{a.d}</div></div>
              </div>
            ))}
          ...
          <CardContent className="space-y-2 text-sm">
            {[
              ...
              <div key={a.t} className="rounded-xl border px-3 py-2">
                <div className="font-medium text-xs">{a.t}</div><div className="text-xs text-zinc-400">{a.d} • {a.by}</div>
          ...
          <CardContent className="space-y-2">
            {AGENTS.slice(0,4).map(a=>(
              <div key={a.name} className="flex items-center gap-3 text-sm border rounded-xl px-3 py-2">
                <div className="flex-1"><div className="font-medium text-xs">{a.name}</div><div className="text-xs text-zinc-400">₹{a.cost} / ₹{a.budget} • {a.jobs} jobs</div><div className="mt-1 h-1.5 rounded-full bg-zinc-800 overflow-hidden"><div className="h-full bg-orange-500" style={{width: `${Math.min(100, (a.cost/a.budget)*100)}%`}} /></div></div>
          ...
          <CardContent className="space-y-2">
            {DELIVERIES.map(d=>(
              <div key={d.id} className="rounded-xl border px-3 py-2 text-sm flex items-center justify-between">
                <div><div className="font-medium text-xs">{d.id} • {d.driver} • {d.orders} orders</div><div className="text-xs text-zinc-400">{d.route}</div></div>
                <Badge variant={d.status==="Delivered"?"success": d.status==="In Transit"?"warning":"secondary"}>{d.status}</Badge>
              </div>
            ))}
            <div className="flex gap-2 text-xs"><span className="inline-flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5 text-orange-500" /> OTP verified POD</span><span className="inline-flex items-center gap-1"><Activity className="h-3.5 w-3.5 text-orange-500" /> GPS live</span></div>
          </CardContent>
        </Card>
      </div>

      <div className="grid lg:grid-cols-3 gap-4">
        <Card><CardHeader className="pb-2"><div className="font-semibold text-sm flex items-center gap-2"><Package className="h-4 w-4" /> Inventory Health</div></CardHeader><CardContent className="space-y-1 text-sm">{INVENTORY.slice(0,3).map(i=>(<div key={i.sku} className="flex justify-between border rounded-lg px-3 py-2 text-xs"><span>{i.name}</span><span className={i.ats<50?"text-red-600 font-bold":"text-zinc-400"}>ATS {i.ats}</span></div>))}<Link href="/dashboard/inventory" className="text-xs underline">Full stock + ATS →</Link></CardContent></Card>
        <Card><CardHeader className="pb-2"><div className="font-semibold text-sm">System Health — Paid SLA</div></CardHeader><CardContent className="grid grid-cols-2 gap-2 text-xs">
          <div className="rounded-xl border p-3 text-center"><div className="font-bold">99.92%</div><div className="text-zinc-400">Uptime 30d</div></div>
          <div className="rounded-xl border p-3 text-center"><div className="font-bold">184 ms</div><div className="text-zinc-400">API p95</div></div>
          <div className="rounded-xl border p-3 text-center"><div className="font-bold">0</div><div className="text-zinc-400">DLQ events</div></div>
          <div className="rounded-xl border p-3 text-center"><div className="font-bold">12 min</div><div className="text-zinc-400">Backup age</div></div>
        </CardContent></Card>
