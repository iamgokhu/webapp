  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2"><Zap className="h-5 w-5 text-orange-500" /> Billing — 8-Second Invoice <Badge variant="success">Paid GST • e-Invoice</Badge></h1>
          <p className="text-xs text-zinc-400">HSN auto, GST calc, e-Way above 50K, IRN via GSP. The fastest billing flow in market — paid includes e-Invoice &amp; e-Way unlimited.</p>
        </div>
      ...
      <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-4">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-3">
              <label className="text-sm font-medium space-y-1"><span>Party (search)</span><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" /><input value={party} onChange={e=>setParty(e.target.value)} className="w-full rounded-xl border border-zinc-700 pl-9 pr-3 py-2 bg-zinc-900 text-white placeholder:text-zinc-500" placeholder="Search party / GSTIN" /></div></label>
              <label className="text-sm font-medium space-y-1"><span>Branch / Godown</span><select className="w-full rounded-xl border border-zinc-700 px-3 py-2 bg-zinc-900 text-white"><option>Salem-A (Default)</option><option>Salem-B</option><option>Erode</option></select></label>
            </div>
              ...
              <div className="divide-y">
                {lines.map((l,i)=>(
                  <div key={i} className="grid grid-cols-[1fr_70px_90px_70px_90px] gap-2 p-2 items-center text-sm">
                    <div className="min-w-0"><div className="font-medium truncate">{l.name}</div><div className="text-xs text-zinc-400">{l.sku} • HSN 1101 • GST {l.gst}%</div></div>
                    <input type="number" value={l.qty} onChange={e=>{const v=[...lines]; v[i].qty=Number(e.target.value); setLines(v)}} className="rounded-lg border border-zinc-700 bg-zinc-900 text-white px-2 py-1.5 text-center" />
                    <input type="number" value={l.price} onChange={e=>{const v=[...lines]; v[i].price=Number(e.target.value); setLines(v)}} className="rounded-lg border border-zinc-700 bg-zinc-900 text-white px-2 py-1.5 text-right" />
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="rounded-xl border p-3 bg-zinc-800/60 space-y-1"><div className="flex justify-between"><span>Subtotal</span><b>{formatCurrency(subtotal)}</b></div><div className="flex justify-between"><span>GST</span><b>{formatCurrency(tax)}</b></div><div className="flex justify-between text-base border-t pt-1"><span>Grand Total</span><b className="text-orange-600">{formatCurrency(total)}</b></div></div>
              <div className="space-y-2">
                <label className="block text-xs font-medium">Payment mode (paid: UTR verify)<select className="mt-1 w-full rounded-xl border border-zinc-700 px-3 py-2 bg-zinc-900 text-white"><option>Cash</option><option>UPI — Verify UTR</option><option>NEFT/RTGS</option><option>Cheque</option></select></label>
                <label className="block text-xs font-medium">Notes<input placeholder="Delivery instructions" className="mt-1 w-full rounded-xl border border-zinc-700 px-3 py-2 bg-zinc-900 text-white" /></label>
              </div>
            </div>
            ...
            <div className="text-xs text-zinc-400 text-center">Order Control Engine validates ATS (Physical - Reserved - Blocked) before invoice. Margin guard enforced.</div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-4">
            ...
            <CardContent className="space-y-2 max-h-[520px] overflow-auto">
              {invoices.map(inv=>(
                <div key={inv.id} className="rounded-xl border p-3 text-sm">
                  ...
                  <div className="flex justify-between text-xs text-zinc-400"><span>{inv.date} • {inv.items} items</span><span className="font-bold">{formatCurrency(inv.amount)}</span></div>
          ...
          <Card className="p-4 bg-amber-50 border-amber-200">
            ...
            <div className="text-xs text-zinc-300 mt-1">Starter: 100 IRN/mo. Growth: 2,000/mo. Enterprise: unlimited + auto GSP retry & Slack alert.</div>
          </Card>
        </div>
      </div>
    </div>
