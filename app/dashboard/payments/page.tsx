...
export default function PaymentsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold">Payments — Ledger & UTR Verification <Badge variant="success">Paid: Auto-match + OCR</Badge></h1>

      <div className="grid lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-2"><div className="font-semibold text-sm">UTR Verification — Multi-factor</div><div className="text-xs text-zinc-500">Prevents reuse, amount mismatch, fake screenshot.</div></CardHeader>
          <CardContent className="space-y-3">
            <div className="flex gap-2">
              <input value={utr} onChange={e=>setUtr(e.target.value)} placeholder="Enter UTR / Ref No" className="flex-1 rounded-xl border px-3 py-2 bg-white font-mono text-sm" />
            ...
            {verified===true && <div className="rounded-xl bg-emerald-50 border border-emerald-200 p-3 text-sm flex gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-600" /> UTR verified • Amount ₹12,400 matches bank statement • Not duplicate.</div>}
            ...
            <div className="rounded-xl border p-3 text-xs space-y-2">
              ...
              <div className="flex gap-2"><Button variant="outline" size="sm" className="gap-1"><Upload className="h-3.5 w-3.5" /> Upload Image</Button><span className="text-zinc-500">Claude Vision extracts UTR, amount, date — cross-checked with statement.</span></div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <div className="pl-4 border-l space-y-1 text-xs">
              <div className="flex justify-between"><span>INV-1841 — ₹12,400</span><span className="font-bold text-emerald-600">✓ Fully paid</span></div>
              <div className="flex justify-between"><span>INV-1839 — ₹12,600 of ₹35,600</span><span className="font-bold">Partial</span></div>
            </div>
            <div className="text-xs text-zinc-500">Cheque tracking: Issued → Deposited → Cleared / Bounced (paid).</div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="space-y-2 text-sm">
          {[
            ...
            <div key={r.id} className="flex items-center justify-between rounded-xl border px-3 py-2">
              <div><div className="font-medium text-xs">{r.id}</div><div className="text-xs text-zinc-500">Due {r.due} • {r.days>0?`${r.days} days overdue`:`Due in ${Math.abs(r.days)} days`}</div></div>
